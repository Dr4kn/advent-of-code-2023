...
fun main() {
    ...
    fun solver(heatMap: List<List<Int>>): Int {
        ...
        while (priorityQueue.isNotEmpty()) {
            ...
            if (node.row == heatMap.size - 1 && node.col == heatMap[0].size -1) return node.heatloss
            ...
            if (Pair(node.movedRow, node.movedCol) != Pair(0, 0) && node.consecutiveMoves < 3) {
                ...
                if (0 <= newRow && newRow < heatMap.size && 0 <= newCol && newCol < heatMap[0].size) {
                    priorityQueue.add(Node(newRow, newCol, node.heatloss + heatMap[newRow][newCol],
            ...
            for (direction in moveDirections) {
                if (direction != Pair(node.movedRow, node.movedCol) && direction != Pair(-node.movedRow, -node.movedCol)) {
    ...
    fun part1(input: List<String>): Int {
        val answer = solver(parseInput(input))
        return answer
    }
